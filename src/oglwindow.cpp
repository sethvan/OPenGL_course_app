...
OGLWindow::OGLWindow( GLint windowWidth, GLint windowHeight )
    : width( windowWidth ), height( windowHeight ), xChange( 0.0f ), yChange( 0.0f ), xoffset( 0.0f ), yoffset( 0.0f ) {
...
void OGLWindow::scroll_callback( GLFWwindow* window, double _xoffset, double _yoffset ) {
   OGLWindow* theWindow = static_cast<OGLWindow*>( glfwGetWindowUserPointer( window ) );
   theWindow->yoffset = _yoffset;
}