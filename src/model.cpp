...
static std::string trim_underscore_num( std::string& filename )
{
    ...
    if ( filename[ posUnderscore - 1 ] == '_' )
    {
        return filename;
    }
    auto posDot = filename.find_last_of( "." );
    if ( posDot < posUnderscore )
    {
        return filename;
    }
    auto length = posDot - posUnderscore;
...
void Model::loadModel( const std::string& filename )
{
    ...
    const aiScene* scene = importer.ReadFile(
        filename, aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_GenSmoothNormals | aiProcess_JoinIdenticalVertices );
    if ( !scene )
    {
...
void Model::printMeshCount() const
{
    std::cout << "Mesh count: " << meshList.size() << '\n';
}
